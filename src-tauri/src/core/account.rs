...
impl AccountProxyConfig {
    pub fn to_proxy_url(&self) -> Result<String, String> {
        ...
        let scheme = match self.protocol {
            ...
            // Use remote DNS resolution for SOCKS5 so upstream host lookups do not bypass the proxy.
        ...
        let username = self.username.as_deref().map(str::trim).filter(|value| !value.is_empty());
...
#[cfg(test)]
mod tests {
    use super::{normalize_accounts, Account, AccountProxyConfig, AccountProxyProtocol};
