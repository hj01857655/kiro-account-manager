...
use crate::commands::common::{
    ...
    is_token_expiring_soon, lock_store, normalize_expires_at, refresh_token_by_provider, save_store,
...
///
/// external_idp 账号的特征：有 clientId、可能无 clientSecret、带微软 tokenEndpoint/issuerUrl，
/// profileArn 必须原样保留（绝不套 BuilderId/social 默认占位，否则上游会返回 400/403）。
/// 当前后端无 M365 刷新链路，导入时用传入的 accessToken 取 usage；取不到不 hard-fail，
/// 账号仍落库。去重口径：external_idp + refresh_token（复用通用 `find_existing_account_idx`）。
#[tauri::command]
#[allow(clippy::too_many_arguments)] // Tauri IPC 命令签名需要显式参数，避免前后端调用契约破坏
pub async fn add_account_by_external_idp(
    ...
    // 取 usage（不刷新微软 token —— 后端暂无 M365 刷新路径）。
    // 构造临时 Account 用于取 usage：provider=None + 真实 profile_arn，
    // resolve_kiro_call_context 会原样保留真实 arn，绝不套占位。
